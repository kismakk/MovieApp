  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!username) {
          const [avatarRes, groupsRes, favouritesRes] = await Promise.all([
          ...
          const groupsRes = await axios.get(dataBaseLink + 'groups/mygroups/' + profileRes.data.userInfo.id_users, { withCredentials: true });
